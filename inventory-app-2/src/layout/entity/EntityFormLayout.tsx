...
export const EntityFormLayout = ({ children, isCompact }: Props) => {
    return (
        <div className={` ${isCompact ? "" : "p-4 sm:p-6"}`}>{children}</div>
    );
};
...
EntityFormLayout.Header = ({ title, helpText, isCompact }: HeaderProps) => {
    return (
        <>
            ...
            {helpText && (
                <p
                    className={`${isCompact ? "pb-4" : "pb-6"} w-full text-sm text-slate-700`}
                >
                    {helpText}
                </p>
            )}
...
EntityFormLayout.Form = ({ onSubmit, children, styled }: FormProps) => {
    return (
        <form
            ...
            className={`${styled ? "p-6 bg-white shadow-sm rounded-xl border border-gray-200" : "pt-2"} `}
            autoComplete="off"
            noValidate
        >
            {children}
...
EntityFormLayout.Actions = ({ children, isCompact }: ActionsProps) => {
    return (
        <div
            className={`flex flex-row gap-6 justify-center  ${isCompact ? "pt-4" : "sm:pt-6 pt-4"} min-w-full`}
        >
            {children}
        </div>
    );
